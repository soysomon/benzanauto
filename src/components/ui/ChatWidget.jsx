...
function TypingDots({ reduceMotion }) {
  return (
    <div className="flex items-center gap-1 px-4 py-3" role="status" aria-live="polite">
      <span className="sr-only">Benzan IA está respondiendo.</span>
      {[0, 1, 2].map(i => (
        <motion.span
          key={i}
          className="w-1.5 h-1.5 rounded-full bg-neutral-400"
          animate={reduceMotion ? undefined : { opacity: [0.3, 1, 0.3], y: [0, -3, 0] }}
          transition={reduceMotion ? undefined : { duration: 0.9, repeat: Infinity, delay: i * 0.18, ease: 'easeInOut' }}
        />
      ))}
    </div>
  )
...
function Message({ msg, onVehicleClick, reduceMotion }) {
...
export default function ChatWidget() {
  ...
  useEffect(() => {
    if (open) {
      ...
      const timeoutId = window.setTimeout(() => inputRef.current?.focus(), reduceMotion ? 0 : 350)
            ...
            >
              ...
              <div className="flex items-center gap-3 px-5 py-4 border-b border-neutral-100 flex-shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  <p id={dialogTitleId} className="font-body font-semibold text-sm text-neutral-900">{COMPANY.assistantName}</p>
                  <div className="flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-green-500" />
                    <p id={dialogDescriptionId} className="font-body text-xs text-neutral-400">En línea · Responde al instante</p>
                  </div>
                </div>
                <button
                  type="button"
              ...
              {/* messages */}
              <div className="flex-1 overflow-y-auto px-4 py-4 space-y-0.5" role="log" aria-live="polite" aria-relevant="additions text">
                {messages.map((msg, i) => (
                  <Message
                    key={i}
                    msg={msg}
              ...
              <div className="flex-shrink-0 border-t border-neutral-100 px-4 py-3">
                <div className="flex items-center gap-2 bg-neutral-50 border border-neutral-200 px-4 py-2.5" style={{ borderRadius: 12 }}>
                  <label htmlFor="benzan-ai-input" className="sr-only">Escribe tu pregunta al asistente Benzan IA</label>
                  <input
                    id="benzan-ai-input"
                    ref={inputRef}
                    type="text"
